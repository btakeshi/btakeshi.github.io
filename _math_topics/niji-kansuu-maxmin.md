...
problems:
  - notation: 学園２０２６春２・大問３（２）
    ...
    a: |
      ...
      &= \colorbox{aqua}{$-$}(x^2\colorbox{misque}{$-4$}x)+1 && \text{\scriptsize \color{aqua}（$x^2$の係数 \colorbox{aqua}{$-1$} でくくる）} \\
      &= -\lbrace(x\colorbox{misque}{$-2$})^2-2^2\rbrace+1 && \text{\scriptsize \color{gray}（$x$の係数 $4$ の半分の$2$を使って平方完成）} \\
